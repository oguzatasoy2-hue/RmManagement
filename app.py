...
<style>
    ...
    * {
    ...
    /* Global Background & Content Area */
    .stApp {
    ...
    /* Elegant Inputs (Buttons, Selectboxes, User Inputs) */
    ...
    button[data-baseweb="tab"][aria-selected="true"] {
        color: #D4AF37 !important; /* Gold Ascent for selected tab */
        border-bottom: 3px solid #D4AF37 !important;
        background-color: rgba(212, 175, 55, 0.05) !important;
    }
    
    /* Primary Buttons (Launch Simulation) */
    button[kind="primary"] {
...
with st.sidebar:
    ...
    mode = st.radio(
        "Mode d'analyse",
        ["📊 Cas d'étude (Le Provençal)", "🧪 Simulation Personnalisée"],
        index=0,
        help="Le cas d'étude utilise les données de l'Hôtel Le Provençal. La simulation vous permet d'entrer les paramètres de votre propre hôtel."
    )
...
if "Simulation" in mode:
    # Mode Simulation : formulaire dans la sidebar
    with st.sidebar:
        st.markdown("### 🧪 Paramètres de Simulation")
        
        sim_name = st.text_input("Nom de l'hôtel", value="Mon Hôtel", max_chars=50)
        sim_city = st.selectbox("Ville", ["Bordeaux", "Paris", "Lyon", "Nice", "Marseille", "Autre"])
        sim_stars = st.slider("Catégorie (étoiles ⭐)", min_value=1, max_value=5, value=3)
        sim_rooms = st.slider("Nombre de chambres", min_value=10, max_value=300, value=50, step=5)
        sim_price = st.slider("Prix de base (€/nuit)", min_value=30, max_value=500, value=85, step=5)
        
        run_sim = st.button("🚀 Lancer la Simulation", use_container_width=True, type="primary")
    
    # Générer ou utiliser le cache session_state
    if run_sim or 'sim_data' not in st.session_state:
        import data_generator
        with st.spinner(f"🔄 Simulation en cours pour **{sim_name}**..."):
            df_raw = data_generator.generate_custom_data(sim_name, sim_stars, sim_rooms, sim_price, sim_city)
            df_raw['Date'] = pd.to_datetime(df_raw['Date'])
            st.session_state['sim_data'] = df_raw
            st.session_state['sim_hotel_name'] = sim_name
    else:
        df_raw = st.session_state.get('sim_data', pd.DataFrame())
        sim_name = st.session_state.get('sim_hotel_name', sim_name)
    
    OUR_HOTEL_NAME = sim_name
    
    # Bannière simulation
    st.info(f"🧪 **Mode Simulation** — Analyse générée pour **{sim_name}** ({sim_stars}⭐, {sim_rooms} ch., {sim_price}€/nuit, {sim_city})")

else:
    # Mode Cas d'étude : chargement du CSV
    df_raw = load_data()
...
if not df_raw.empty:
    ...
    with st.sidebar:
        ...
        if st.button("🔄 Check Live Data (API Booking)"):
            with st.spinner("Interrogation des serveurs Booking en cours..."):
                ...
                st.sidebar.success(f"Données Live récupérées avec succès !")
    ...
    with tab2:
        ...
        for comp in selected_comps:
            ...
            if not comp_df.empty:
                ...
                # Les concurrents n'ont pas de colonne Capacity dans le CSV, on utilise une valeur par défaut
                try:
                    comp_capacity = int(comp_df['Capacity'].iloc[0]) if 'Capacity' in comp_df.columns and pd.notna(comp_df['Capacity'].iloc[0]) else 60
    ...
    with tab3:
        ...
        # Préparer les données : convertir Event en binaire Oui/Non pour le color_map
        df_3d = df_our.copy()
        df_3d['Événement'] = df_3d['Event'].apply(lambda x: 'Oui' if x != 'Non' else 'Non')
